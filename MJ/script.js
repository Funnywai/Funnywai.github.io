...
class MahjongScoreSystem {
    ...
    processWin() {
        ...
        if (this.currentSelections.winType === 'self-draw') {
            ...
            const winAmount = (score + 5) * 3; // 從三家各獲得
            const loseAmount = score + 5; // 每家輸的分數
    ...
    updateHistory() {
        ...
        recentHistory.forEach(entry => {
            ...
            if (entry.winType === 'self-draw') {
                ...
                scoreDetails = `${winnerName} +${(entry.score + 5) * 3}`;
                ...
                for (let i = 0; i < 4; i++) {
                    if (i !== entry.winner) {
                        ...
                        otherPlayers.push(`${playerName} -${entry.score + 5}`);
                    }
                }
                scoreDetails += ` (${otherPlayers.join(', ')})`;
            } else {
                details = `${winnerName} 食胡 ${loserName} ${entry.score} 番`;
                scoreDetails = `${winnerName} +${entry.actualScore} | ${loserName} -${entry.actualScore}`;
            }
...
class MahjongScoreSystem {
    ...
    initializeTouchEvents() {
        ...
        // 為觸摸設備添加額外的視覺反饋
        const touchableElements = document.querySelectorAll('.btn, .score-btn, .player-option, .win-type-option');
    ...
    // 修改現有的 showSuccessMessage 方法，使其在手機上更明顯
    showSuccessMessage() {
        const message = document.createElement('div');
        message.className = 'success-message';
        message.textContent = '分數已更新！';
...
document.addEventListener('DOMContentLoaded', () => {
    // 添加觸摸樣式
    MahjongScoreSystem.addTouchStyles();
